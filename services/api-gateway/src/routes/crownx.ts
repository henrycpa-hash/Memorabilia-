...
export function registerCrownxRoutes(app: FastifyInstance) {
  ...
  app.get("/api/athletes/:id/latest-appraisal", async (request, reply) => { const { id } = request.params as { id: string }; const r = await proxy("GET", `${athBase()}/athletes/${id}/latest-appraisal`); reply.code(r.status).header("content-type", r.ctype).send(r.text); });
  ...
  // appraiser NETWORK (choose your appraiser) + human-in-the-loop queue
  app.get("/api/appraisers", async (request, reply) => { const { specialty } = request.query as { specialty?: string }; const r = await proxy("GET", `${athBase()}/appraisers${specialty ? `?specialty=${encodeURIComponent(specialty)}` : ""}`); reply.code(r.status).header("content-type", r.ctype).send(r.text); });
  app.get("/api/appraisers/:appraiserId", async (request, reply) => { const { appraiserId } = request.params as { appraiserId: string }; const r = await proxy("GET", `${athBase()}/appraisers/${appraiserId}`); reply.code(r.status).header("content-type", r.ctype).send(r.text); });
  app.get("/api/appraisals", async (request, reply) => { const { status, appraiserId } = request.query as { status?: string; appraiserId?: string }; const qs = [status ? `status=${status}` : "", appraiserId ? `appraiserId=${appraiserId}` : ""].filter(Boolean).join("&"); const r = await proxy("GET", `${athBase()}/appraisals${qs ? `?${qs}` : ""}`); reply.code(r.status).header("content-type", r.ctype).send(r.text); });
  app.get("/api/appraisals/:appraisalId/verify", async (request, reply) => { const { appraisalId } = request.params as { appraisalId: string }; const r = await proxy("GET", `${athBase()}/appraisals/${appraisalId}/verify`); reply.code(r.status).header("content-type", r.ctype).send(r.text); });
  ...
  app.post("/api/ai-modeling/governance/board-alloc", async (request, reply) => { const r = await proxy("POST", `${amBase()}/ai-modeling/governance/board-alloc`, request.body); reply.code(r.status).header("content-type", r.ctype).send(r.text); });
